...
class DataprocClusterCreateOperator(BaseOperator):
    """
    Create a new cluster on Google Cloud Dataproc. The operator will wait until the creation is successful or an error
    occurs in the creation process.
    ...
    def __init__(self,
        ...
        :param project_id: The ID of the google cloud project in which to create the cluster
        :type project_id: string
        :param num_workers: The # of workers to spin up
        :type num_workers: int
        :param storage_bucket: The storage bucket to use, setting to None lets dataproc generate a custom one for you
        :type storage_bucket: string
        :param init_actions_uris: List of GCS uri's containing dataproc initialization scripts
        :type init_actions_uris: list[string]
        :param metadata: dict of key-value google compute engine metadata entries to add to all instances
        :type metadata: dict
        :param properties: dict of properties to set on config files (e.g. spark-defaults.conf), see
    ...
    def _cluster_ready(self, state, service):
        ...
        if state == 'ERROR':
            ...
            try:
                ...
                error_details = 'Unknown error in cluster creation, check Google Cloud console for details.'
    ...
    def _wait_for_done(self, service):
        while True:
            ...
            if state is None:
                logging.log(logging.INFO, "No state for cluster '%s'", self.cluster_name)
                time.sleep(15)
            else:
                logging.log(logging.INFO, "State for cluster '%s' is %s", self.cluster_name, state)
                if self._cluster_ready(state, service):
                    logging.log(logging.INFO, "Cluster '%s' successfully created", self.cluster_name)
                    return
                time.sleep(15)
    ...
    def execute(self, context):
        ...
        if self._get_cluster(service):
            logging.log(logging.INFO, 'Cluster {} already exists... Checking status...'.format(self.cluster_name))
        ...
        if self.init_actions_uris:
            init_actions_dict = [{'executableFile': uri} for uri in self.init_actions_uris]
            cluster_data['config']['initializationActions'] = init_actions_dict

        try:
            request = service.projects().regions().clusters().create(
                projectId=self.project_id,
                region=self.region,
                body=cluster_data
            )
